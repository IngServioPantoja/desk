...
if(empty($lista_res)){
    ...
    <td><?php echo $html->link('Datos Personales',array('controller'=>'datos_personales','action'=>'editar_p',$persona['datos_personales']['id']));
              echo "\t".$html->link('Cuenta de Usuario',array('controller'=>'datos_personales','action'=>'editar_c',$persona['datos_personales']['id']));
              echo "\t".$html->link('Inhabilitar',array('controller'=>'datos_personales','action'=>'inhabilitar',$persona['datos_personales']['id']),array(),'Esta seguro que desea inhabilitar este usuario? (Al inhabilitarlo, el usuario no podra iniciar sesion y sus datos no apareceran en las consultas dentro del sistema)');  ?></td>
    </tr>
    <?php
    $i++;
    endforeach;
